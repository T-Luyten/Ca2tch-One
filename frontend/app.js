...
const S = {
  ...
  roiCircleDrawing: null,  // { role, mousePos:[x,y], seed:[x,y]|null, dilation, roiId }
...
const D = {
  ...
  useSourceAsMeasureBtn: $('use-source-as-measure-btn'),
  ...
  roiCircleAddBtn:$('roi-circle-add-btn'),
  roiMergeBtn:   $('roi-merge-btn'),
  roiDeleteBtn:  $('roi-delete-btn'),
  roiCancelBtn:  $('roi-cancel-btn'),
  roiCircleCancelBtn:$('roi-circle-cancel-btn'),
  roiDrawHint:   $('roi-draw-hint'),
  roiCircleDrawHint:$('roi-circle-draw-hint'),
...
function init() {
  ...
  for (const role of Object.keys(D.viewers)) {
    ...
    viewer.roiCanvas.addEventListener('wheel', e => onCanvasWheel(role, e), { passive: false });
  }
  ...
  D.useSourceAsMeasureBtn.addEventListener('click', useSourceAsMeasurement);
  ...
  D.cellMarginPx.addEventListener('change', () => {
    invalidateAnalysis('', { preserveRaw: true });
  });
  D.widthFraction.addEventListener('change', () => {
    invalidateAnalysis('', { preserveRaw: true });
  });
  ...
  D.baselinePercentile.addEventListener('change', () => {
    invalidateAnalysis('', { preserveRaw: true });
  });
  ...
  D.roiCircleAddBtn.addEventListener('click', startCircularRoiDraw);
  ...
  D.roiCircleCancelBtn.addEventListener('click', cancelCircularRoiDraw);
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') {
      cancelBGDraw();
      cancelManualRoiDraw();
      cancelCircularRoiDraw();
    }
    if (e.key === 'Enter' && S.roiCircleDrawing) {
      e.preventDefault();
      if (S.roiCircleDrawing.seed) {
        finishCircularRoiDraw();
      }
    }
  });
...
function syncButtons() {
  ...
  D.useSourceAsMeasureBtn.disabled = !(sourceFile().fileId && !measureFile().fileId);
  D.analyzeBtn.disabled = !(measureFile().fileId && selectedAnalysisRoiIds().length);
  D.rulerBtn.disabled = !sourceFile().fileId;
  D.roiAddBtn.disabled = !sourceFile().fileId || S.bgDrawing || !!S.roiDrawing || !!S.roiCircleDrawing || S.ruler.active;
  D.roiCircleAddBtn.disabled = !sourceFile().fileId || S.bgDrawing || !!S.roiDrawing || !!S.roiCircleDrawing || S.ruler.active;
  D.roiMergeBtn.disabled = !sourceFile().fileId || !!S.roiDrawing || !!S.roiCircleDrawing || sourceSelectedIds.length < 2;
  D.roiDeleteBtn.disabled = !sourceFile().fileId || !!S.roiDrawing || !!S.roiCircleDrawing || sourceSelectedIds.length === 0;
  D.roiCancelBtn.style.display = S.roiDrawing ? 'block' : 'none';
  D.roiDrawHint.style.display = S.roiDrawing ? 'block' : 'none';
  D.roiCircleCancelBtn.style.display = S.roiCircleDrawing ? 'block' : 'none';
  D.roiCircleDrawHint.style.display = S.roiCircleDrawing ? 'block' : 'none';
}
...
async function useSourceAsMeasurement() {
  const source = sourceFile();
  if (!source.fileId || measureFile().fileId) return;

  D.useSourceAsMeasureBtn.disabled = true;
  D.transferStatus.textContent = 'Duplicating source file as measurement…';
  try {
    const res = await apiFetch(`/api/duplicate/${source.fileId}`, { method: 'POST' });

    const measure = measureFile();
    measure.fileId   = res.file_id;
    measure.fileName = source.fileName;
    measure.metadata = res.metadata;
    measure.frame    = 0;
    measure.channel  = 0;
    measure.displayMode = 'channel';
    measure.displayChannel = 0;
    measure.cmin     = res.initial_contrast.min;
    measure.cmax     = res.initial_contrast.max;
    measure.rois     = [];
    measure.autoColormap = true;
    ...
    D.baselineEnd.value = 0;
    D.aucStart.value = 0;
    D.aucStart.max   = res.metadata.n_frames - 1;
    D.aucEnd.value   = 0;
    D.aucEnd.max     = res.metadata.n_frames;
    D.tgFrame.max = res.metadata.n_frames - 1;
    D.addbackFrame.max = res.metadata.n_frames - 1;
    D.tgEndFrame.max = res.metadata.n_frames;
    D.addbackEndFrame.max = res.metadata.n_frames;
    D.tgFrame.value = 0;
    D.addbackFrame.value = 0;
    D.tgEndFrame.value = 0;
    D.addbackEndFrame.value = 0;
    ...
    D.transferStatus.textContent = 'Source file duplicated as measurement.';
  } catch (err) {
    D.transferStatus.textContent = `Error: ${err.message}`;
  } finally {
    syncButtons();
...
function drawROIs(role = S.activeRole) {
  ...
  // ── In-progress circular ROI ──
  if (S.roiCircleDrawing?.role === role) {
    const { mousePos, seed } = S.roiCircleDrawing;
    if (!seed) {
      // Preview crosshair before first click
      const cx = mousePos[0] * sc;
      const cy = mousePos[1] * sc;
...
function onCanvasMove(role, e) {
  ...
  if (S.roiCircleDrawing?.role === role) {
    S.roiCircleDrawing.mousePos = [x, y];
    drawROIs(role);
    return;
  }
...
function onCanvasClick(role, e) {
  ...
  if (S.roiCircleDrawing?.role === role) {
    const { x, y } = canvasXY(role, e);
    if (!S.roiCircleDrawing.seed) {
      S.roiCircleDrawing.seed = [x, y];
      runAutoDetect(role, [x, y], 0, null);
    } else {
      finishCircularRoiDraw();
    }
    return;
...
function startCircularRoiDraw() {
  ...
  cancelManualRoiDraw();
  S.activeRole = 'source';
  S.roiCircleDrawing = { role: 'source', mousePos: [0, 0], seed: null, dilation: 0, roiId: null };
  D.viewers.source.roiCanvas.classList.add('drawing-bg');
  D.roiCircleAddBtn.textContent = 'Detecting Cell…';
  syncButtons();
  drawROIs('source');
}

async function runAutoDetect(role, seed, dilation, replaceRoiId) {
  const file = S.files[role];
  try {
    const res = await apiFetch(`/api/roi/${file.fileId}/autodetect`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ seed, radius: 60, dilation, replace_roi_id: replaceRoiId }),
    });
    ...
    if (role === 'source') {
      measureFile().rois = [];
      D.transferStatus.textContent = measureFile().fileId
        ? 'Auto-detected ROI added on ROI source. Copy ROIs to the measurement file before analysis.'
        : 'Load a measurement file to transfer these ROIs.';
      drawROIs('measure');
    }
    clearAnalysisState();
    ...
    renderROIList();
    drawROIs(role);
    setStatus(`Auto-detected ROI ${res.roi.id}. Scroll to adjust, Enter to keep, Esc to discard.`);
    if (S.roiCircleDrawing) {
      S.roiCircleDrawing.roiId = res.roi.id;
    }
  } catch (err) {
    setStatus(`Auto-detect error: ${err.message}`);
  }
}

function cancelCircularRoiDraw() {
  if (!S.roiCircleDrawing) return;
  const { role, roiId } = S.roiCircleDrawing;
  if (roiId != null) {
    deleteROI(roiId);
  }
  D.viewers[role].roiCanvas.classList.remove('drawing-bg');
  S.roiCircleDrawing = null;
  D.roiCircleAddBtn.textContent = 'Auto-Detect Cell';
  syncButtons();
  drawROIs(role);
}

function finishCircularRoiDraw() {
  if (!S.roiCircleDrawing) return;
  const { role } = S.roiCircleDrawing;
  D.viewers[role].roiCanvas.classList.remove('drawing-bg');
  S.roiCircleDrawing = null;
  D.roiCircleAddBtn.textContent = 'Auto-Detect Cell';
  syncButtons();
...
function onCanvasWheel(role, e) {
  if (S.roiCircleDrawing?.role !== role) return;
  if (!S.roiCircleDrawing.seed) return;
  e.preventDefault();
  const step = e.deltaY > 0 ? -1 : 1;
  S.roiCircleDrawing.dilation = Math.max(-10, Math.min(10, S.roiCircleDrawing.dilation + step));
  runAutoDetect(role, S.roiCircleDrawing.seed, S.roiCircleDrawing.dilation, S.roiCircleDrawing.roiId);
}
...
async function runAnalysis() {
  ...
  const body = {
    ...
    baseline_percentile: +D.baselinePercentile.value,
