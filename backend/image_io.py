...
def load_nd2_file(filepath: str):
    ...
    with nd2.ND2File(filepath) as f:
        ...
        # Pixel size (XYZ voxel size in micrometres)
        pixel_size = None
        try:
            voxel = f.voxel_size()
...
def load_czi_file(filepath: str):
    ...
    NOTE: Two CZI libraries are used intentionally:
    - aicspylibczi reads the image array and dimension order reliably.
    - czifile provides lightweight XML metadata and hardware timestamps.
    Consolidating onto a single library would be a future refactor.
    """
...
def _normalize_shape(data, sizes):
    """Ensure data is (T, C, Y, X).

    - Z is collapsed by max projection.
    - Singleton unsupported axes are safely dropped.
    - Non-singleton unsupported axes (e.g. multi-scene 'S') raise an error so
    ...
    # Drop unsupported axes. Singleton axes are safe to drop; anything larger
    # would silently throw away data (e.g. multiple scenes/positions/rotations).
    dropped_non_singleton = {}
    for i in range(len(axes) - 1, -1, -1):
        axis_name = axes[i]
        if axis_name in {'T', 'C', 'Y', 'X'}:
            continue
        axis_size = sizes[axis_name]
        extra_axes[axis_name] = axis_size
        if axis_size == 1:
            data = np.take(data, indices=0, axis=i)
            axes.pop(i)
        else:
            dropped_non_singleton[axis_name] = axis_size

    if dropped_non_singleton:
        raise ValueError(
            "Unsupported non-singleton axes would be silently discarded. "
            f"Please open a single scene/position/rotation or export the data "
            f"as T,C,Y,X. Offending axes: {dropped_non_singleton}. "
            f"Z was collapsed; singleton axes were dropped."
        )
...
def get_ratio_projection(data, proj_type='mean', ch_num=0, ch_den=1):
    """Project a ratiometric channel pair over time.

    This computes the pixel-wise ratio F_num / F_den for each frame and then
    projects those ratios over time. Note that this is not the same as
    projecting each channel first and then taking the ratio; the two conventions
    differ when the denominator is noisy or near zero in individual frames.
    """
