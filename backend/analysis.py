...
# but is deprecated in NumPy 2.0+.
import warnings
try:
...
def compute_background_trace(ch_data, labels, mode, percentile=50.0, bg_mask=None, cell_margin_px=_CELL_MARGIN_PX):
    ...
    bg_trace = []
    # percentile is validated by the API (0–100); keep as float for numpy.
...
def extract_ratio_traces(data, labels, roi_ids,
    ...
    for roi_id in roi_ids:
        ...
        # Guard against near-zero or negative denominators. Fluorescence after
        # background subtraction should be non-negative, so only positive
        # denominators produce physically meaningful ratios.
        _eps = 1e-9
...
def compute_delta_f(traces, baseline_start=0, baseline_end=10, baseline_percentile=8.0):
    """
    Compute ΔF/F₀ for each ROI trace.

    F₀ = percentile of frames [baseline_start, baseline_end).  Default 8th
    percentile follows common calcium-imaging practice: it approximates the
    resting fluorescence and is robust to occasional transients in the baseline
    window.  Use 50 for the median or 0 for the mean (not recommended).

    ΔF/F₀ = (F − F₀) / F₀
    """
    baseline_percentile = float(np.clip(baseline_percentile, 0.0, 100.0))
    delta_f = {}
    for roi_id, trace in traces.items():
        ...
        baseline_window = arr[start:end]
        if baseline_percentile == 0.0:
            f0 = float(baseline_window.mean())
        else:
        ...
        if f0 > 0:
            ...
            logger.warning(
                'ROI %s: F₀ = %s is not positive (baseline window [%s:%s], percentile=%s). '
                'Returning NaN for ΔF/F₀.',
                roi_id, f0, start, end, baseline_percentile
            )
...
def _max_rise_rate(window, x):
    """Maximum rate of rise using a Savitzky-Golay smoothed derivative.

    The raw finite-difference max slope is extremely sensitive to frame-to-frame
    noise.  A small SG filter preserves the transient kinetics while giving a
    stable derivative.  Falls back to the raw difference if the window is too
    short for SG smoothing.
    """
    ...
    # Interpolate over NaNs so the smoother can run, but only use time points
    # where the original data were valid when computing the final slope.
    t_idx = np.arange(len(window))
    smoothed = window.astype(float, copy=True)
    if not valid.all():
        smoothed[~valid] = np.interp(t_idx[~valid], t_idx[valid], window[valid])

    n = len(smoothed)
    if n >= 5:
        # A small window preserves fast calcium transient kinetics while
        # suppressing single-frame noise.  5–7 frames is a pragmatic default.
        sg_window = max(5, min(n, 7))
        if sg_window % 2 == 0:
            sg_window -= 1
        try:
            smoothed = savgol_filter(smoothed, window_length=sg_window, polyorder=min(3, sg_window - 1))
        except Exception:
            pass  # fall back to raw below
    ...
    slopes = np.diff(smoothed)[good] / dt[good]
    return float(np.nanmax(slopes)) if slopes.size else 0.0
...
def _event_fwhm(window, x, peak_idx, width_fraction=0.5, baseline_level=None):
    """Estimate event width at a fraction of peak amplitude above baseline.

    Parameters
    ----------
    baseline_level : float | None
        Pre-event baseline to use. If None, falls back to the minimum value
        before the peak for backward compatibility, which can underestimate the
        width when a previous event has not fully decayed.
    """
...
def _event_onset_time(window, x, peak_idx, baseline_level=None, onset_fraction=0.1, local_window=5):
    """Estimate event onset time by backward search from the peak.

    Parameters
    ----------
    baseline_level : float | None
        Global baseline hint. If provided, it is used to find a rough onset
        crossing first; the final onset is then computed relative to a local
        baseline estimated just before that rough onset. If None, the local
        baseline is estimated from the frames immediately preceding the peak
        (best when the global baseline is unreliable).
    local_window : int
        Number of frames before the rough onset used to estimate the local
        baseline.
    """
    peak_value = window[peak_idx]
    ...
    # Step 1: find a rough onset crossing. Use the provided baseline if
    # available, otherwise the median of a short pre-peak window.
    if baseline_level is not None:
        rough_baseline = float(baseline_level)
    else:
        pre_peak = window[max(0, peak_idx - local_window):peak_idx + 1]
        rough_baseline = float(np.nanmedian(pre_peak)) if pre_peak.size else 0.0

    rough_amplitude = peak_value - rough_baseline
    if rough_amplitude <= 0:
        return float('nan')
    rough_onset_level = rough_baseline + onset_fraction * rough_amplitude

    rough_onset_idx = None
    for i in range(peak_idx, 0, -1):
        y_prev = window[i - 1]
        y_curr = window[i]
        if np.isnan(y_prev) or np.isnan(y_curr):
            continue
        if y_prev <= rough_onset_level <= y_curr:
            rough_onset_idx = i
            break

    if rough_onset_idx is None:
        return float('nan')

    # Step 2: estimate a local baseline from a short window just before the
    # rough onset. This removes bias from slow drift or residual signal from a
    # previous event.
    local_start = max(0, rough_onset_idx - local_window)
    local_vals = window[local_start:rough_onset_idx]
    if local_vals.size == 0:
        return float('nan')
    local_baseline = float(np.nanmedian(local_vals))

    local_amplitude = peak_value - local_baseline
    if local_amplitude <= 0:
        return float('nan')
    onset_level = local_baseline + onset_fraction * local_amplitude
...
def _event_decay_tau(window, x, peak_idx, baseline_level=None):
    ...
    ----------
    baseline_level : float | None
        Pre-event baseline (asymptote) to use for the decay fit. If None,
        falls back to the minimum of the falling phase for backward
        compatibility, though that is not recommended.
    """
    peak_value = window[peak_idx]
    if np.isnan(peak_value) or peak_value <= 0:
        return float('nan')
    ...
    if baseline_level is None:
        ...
        baseline = float(baseline_level)
    # The asymptote must be below the peak; otherwise the decay model is invalid.
    if baseline >= peak_value:
        return float('nan')
    above = tw - baseline
    positive = above > 0
    if positive.sum() < 4:
...
def _event_decay_half_time(window, x, peak_idx, baseline_level=None):
    """Estimate decay half-time (t½) from the falling phase.

    Parameters
    ----------
    baseline_level : float | None
        Pre-event baseline to use. If None, falls back to the minimum of the
        falling phase for backward compatibility, though that can underestimate
        the decay when the tail has not fully returned to baseline.
    """
    peak_value = window[peak_idx]
    if np.isnan(peak_value) or peak_value <= 0:
        return float('nan')
...
def compute_summary_metrics(
    ...
    # No analysis window explicitly set → skip event calculations
    if int(auc_start) == 0 and int(auc_end) == 0:
        empty_list = {}
        return peaks, aucs, durations, frequencies, rise_times, time_to_peaks, decays, decay_taus, rise_rates, empty_list
    ...
    for roi_id, trace in traces.items():
        ...
        # Prominence should scale with the noise level and the user's threshold
        # multiplier, not be fixed to one MAD. This matches the height threshold
        # (median + multiplier * MAD) and avoids rejecting small real events.
        prominence = max(
            (threshold_std_multiplier * mad) if baseline.size else 0.0, 1e-9
        )
        ...
        if x.size:
            for idx in peak_indices:
                ...
                onset_time = _event_onset_time(window, x, idx, baseline_level=None, onset_fraction=onset_fraction)
                width = _event_fwhm(window, x, idx, width_fraction=width_fraction, baseline_level=baseline_level)
                decay = _event_decay_half_time(window, x, idx, baseline_level=baseline_level)
                ...
                if compute_decay_tau:
                    tau = _event_decay_tau(window, x, idx, baseline_level=baseline_level)
                    if np.isfinite(tau):
        ...
        rise_rates[roi_id] = _max_rise_rate(window, x)
...
def _stimulus_response_metrics(arr, t, stim_frame, end_frame, baseline_frames=5, slope_frames=5):
    ...
    slope = 0.0
    if slope_end >= 2:
        # Skip the first post-stimulus frame to avoid the immediate addition
        # artifact (pipetting/perfusion delay) when enough frames remain.
        # If the window is very short, keep frame 0 so polyfit still has data.
